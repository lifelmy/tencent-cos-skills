...
1. **不修改用户配置**: 技能不应该自动修改用户的 `~/.cos.yaml` 配置文件，除非用户明确要求
2. **首次配置引导**: 第一次使用时，引导用户设置默认存储桶和上传路径（详见 `references/first-time-setup.md`）
3. **后续直接使用**: 配置完成后，后续操作直接使用默认值，无需重复询问
4. **用户可覆盖**: 用户可以在任何操作中指定其他值，覆盖默认配置
5. **示例配置**: 不要在用户配置文件中添加示例配置
